...
const argv = require('yargs')
    ...
    .option({
        ...
        syncmode: {
            demand: false,
            requiresArg: true,
            describe: 'Gjpy synchronization mode: [fast|light|full]',
            nargs: 1,
            type: 'string',
            group: 'Mist options:',
        },
        ...
        '': {
            describe: 'To pass options to the underlying node (e.g. Gjpy) use the --node- prefix, e.g. --node-datadir',
            group: 'Node options:',
        },
    })
    .help('h')
...
class Settings {
    ...
    get appName() {
        return this.uiMode === 'mist' ? 'Mist' : 'Japariyen Wallet';
    }
    ...
    get rpcIpcPath() {
        ...
        if (process.platform === 'darwin') {
            ipcPath += '/Library/Japariyen/gjpy.ipc';
        } else if (process.platform === 'freebsd' ||
       process.platform === 'linux' ||
       process.platform === 'sunos') {
            ipcPath += '/.ethereum/gjpy.ipc';
        } else if (process.platform === 'win32') {
            ipcPath = '\\\\.\\pipe\\gjpy.ipc';
        }
